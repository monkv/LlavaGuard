...
def evaluate_dataset(model_dir: str, mode: str, data_path: str, output_dir: str, responses_path: str):
    ...
    with open(responses_path, "r", encoding="utf-8") as f:
        model_resp = json.load(f)

    # build index: id → model prediction
    pred_index = {item["id"]: item["model repsonse"] for item in model_resp}
    ...
    for eval_item in gt_data:
        sample_id = eval_item["id"]
        # extract GT (string)
        gt = eval_item["conversations"][1]["value"]
        # find prediction
        if sample_id not in pred_index:
            print(f"WARNING: No prediction found for sample {sample_id}, skipping.")
            continue
        prediction = pred_index[sample_id]

        # update metrics
        emc.add_sample(mode, sample_id, prediction, gt)
...
if __name__ == "__main__":
    ...
    output_dir = args.output_dir if args.output_dir is not None else f"{local_data_dir}/PEGI-Guard-DS/v2/eval/pegiguard/qwen_assessment_v2.json"
